...
from .softmax_free_transformer import (
    SoftmaxFreeTransformerBlock
)
...
class SoftmaxFreeVisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x = x + pos_embed1
        x = self.pos_drop1(x)
        for name, blk in self.block1.named_children():
            x = blk(x, H, W)
        ...
        x = x + pos_embed2
        x = self.pos_drop2(x)
        for name, blk in self.block2.named_children():
            x = blk(x, H, W)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        ...
        x = x + pos_embed3
        x = self.pos_drop3(x)
        for name, blk in self.block3.named_children():
            x = blk(x, H, W)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
